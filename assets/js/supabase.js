  ...
  <div class="spotlight-body">
    ...
    ${bio ? `
      <div class="sc-bio">
        ...
        ${bio.length > 120 ? `<span class="sc-bio-full" style="display:none;">${bio}</span>
        <a href="#" class="sc-toggle" onclick="this.previousElementSibling.style.display='inline';this.previousElementSibling.previousElementSibling.style.display='none';this.style.display='none';return false;" style="color:var(--gold);font-size:0.8rem;display:block;margin-top:0.3rem;">Read more ↓</a>` : ''}
      </div>` : ''}
...
function renderSponsorTicker(sponsors) {
  ...
  // Venuewise house slide — appears every 3rd rotation
  ...
  // Inject Venuewise every 3rd slot
  const allSlides = [];
  sponsors.forEach((s, i) => {
    allSlides.push(s);
    if ((i + 1) % 2 === 0) allSlides.push(venuewiseSlide);
  });
  if (!allSlides.some(s => s.sponsor_name === 'Venuewise')) allSlides.push(venuewiseSlide);

  let current = 0;
  const slides = allSlides.map(s => {
